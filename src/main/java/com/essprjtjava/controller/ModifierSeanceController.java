...
public class ModifierSeanceController {
    ...
    private void initFormulaire() {
        try {
            ...
            cbMatiere.setItems(FXCollections.observableArrayList(matiereService.getAllMatieres()));
            cbSalle.setItems(FXCollections.observableArrayList(salleService.getAllSalles()));
    ...
    private void handleEnregistrerModif() {
        ...
        try {
            ...
            seanceModif.setMatiere(cbMatiere.getValue());
            seanceModif.setSalle(cbSalle.getValue());

            // Utiliser le service qui gère les conflits
            seanceService.modifierSeance(seanceModif);
    ...
    private void handleSupprimerSeance() {
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            try {
                seanceService.supprimerSeance(seanceSelectionnee.getIdSeance());
    ...
    private boolean validerChamps() {
        if (cbJour.getValue() == null || cbHeureDebut.getValue() == null ||
                cbHeureFin.getValue() == null || cbMatiere.getValue() == null ||
